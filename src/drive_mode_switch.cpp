...
#include "stall_detection_header.h"
...
void stall_control_cb() {
	...
	if (mode == drive_mode::STALLING) {
		...
		if (stallDetector.getExpectedVelocity() <= stallDetector.getMaxSpeed)//if we are, try going forward again
		{
...
int main(int argc, char **argv) {
	...
	//stall detection
	{
		...
		while (ros::ok())
		{
			...
			ros::spin();
			ptime lastStuckTime(stallDetector.getLastStuckTime() + td);
